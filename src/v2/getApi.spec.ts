...
describe('getApi', () => {
  ...
  it('handles res type ref', () => {
    ...
    expect(api.resType.value).toBe('AreaItem')
  })
  ...
  it('handles res type void', () => {
    ...
    expect(api.resType.value).toBe('void')
  })
  ...
  it('collects ref deps and dedup', () => {
    ...
    expect(api.typeSymbolDeps).toEqual(['A', 'B', 'C'])
  })
  ...
  it('handles parameter with access paths', () => {
    ...
    expect(api.parameters.find(e => e.identifier === 'requestHeader')?.type.value === 'any')
    expect(api.parameters.find(e => e.identifier === 'members')?.type.value === 'any')
  })
  ...
  it('extracts parameter object', () => {
    ...
    expect(api.paramsType.value).toBe('FetchThingsParams')
    expect(api.extracts[0].identifier).toBe(api.paramsType.value)
  ...
  it('doesnt extracts parameter object when one param and it is not ref', () => {
    ...
    expect(api.paramsType.value).toBe('A')
  })
